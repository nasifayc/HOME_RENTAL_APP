...
class UserRepository implements IUserRepository {
  ...
  @override
  Future<Either<UserError?, RateLoaded?>> fetchRate(String id) async {
    try {
      final prefs = await SharedPreferences.getInstance();
      final refreshToken = prefs.getString("refreshToken") ?? '';
      String accessToken = prefs.getString("accessToken") ?? '';
      var response = await http.get(
        Uri.parse("$baserURL/api/v1/auth/users/$id/rate"),
        headers: {
          "Authorization": "Bearer $accessToken",
          "Content-Type": "application/json"
        },
      ...
      if (response.statusCode == 200) {
        ...
        return Right(RateLoaded(rate: data["rate"]));
      } else {
        return Left(UserError('failed to load profile'));
      }
    } catch (e) {
  ...
  @override
  Future<Either<UserError?, String?>> rateUser(num amount, String id) async {
    try {
      final prefs = await SharedPreferences.getInstance();
      final refreshToken = prefs.getString("refreshToken") ?? '';
      String accessToken = prefs.getString("accessToken") ?? '';
      ...
      if (response.statusCode == 401 || response.statusCode == 403) {
        ...
        if (refreshResponse.statusCode == 201) {
          ...
          response = await http.patch(
            Uri.parse("$baserURL/api/v1/auth/users/$id"),
            headers: {
              "Authorization": "Bearer $accessToken",
              "Content-Type": "application/json"
            },
      ...
      if (response.statusCode == 200) {
        return const Right("Rated Successfully");
      } else {
        return Left(UserError('failed to load profile'));
      }
    } catch (e) {
